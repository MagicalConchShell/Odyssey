...
/**
 * MarkdownEditor component for editing the global CLAUDE.md system prompt
 * 
 * @example
...
export const MarkdownEditor: React.FC<MarkdownEditorProps> = ({
  ...
  const loadSystemPrompt = async () => {
    try {
      setLoading(true);
      setError(null);
      const result = await window.electronAPI.fileSystem.getSystemPrompt();
      if (result.success) {
        setContent(result.content || "");
        setOriginalContent(result.content || "");
      } else {
        throw new Error(result.error || "Failed to load system prompt");
      }
    } catch (err) {
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Edit your global Claude Code system prompt
              </p>
            </div>
          </div>
